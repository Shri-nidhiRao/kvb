document.addEventListener("DOMContentLoaded", () => {
    ...
    const renderBlocks = () => {
        ...
        if (window.blogBlocks.length === 0) {
            blockContainer.innerHTML = '<div style="text-align:center; padding: 2rem; color:#aaa; border: 1px dashed #ddd; border-radius: 8px;">No blocks added yet. Use the buttons below to start building.</div>';
            return;
        }
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                closeModal();
                fetchBlogs();
                showToast(id ? "Blog updated successfully!" : "Blog published successfully!");
            } else {
    ...
    const parseLegacyHTML = (htmlStr) => {
        ...
        if (children.length === 0) {
            ...
            if (raw) blocks.push({ type: 'paragraph', value: raw.replace(/<br\s*[\/]?>/gi, '\n').replace(/<\/?[^>]+(>|$)/g, "") });
            return blocks;
        }
        ...
        children.forEach(el => {
            ...
            if (['H1', 'H2', 'H3', 'H4', 'H5', 'H6'].includes(tag)) {
                ...
                const liItems = Array.from(el.querySelectorAll('li')).map(li => li.textContent.trim());
                if (liItems.length > 0) {
        ...
        return blocks.length > 0 ? blocks : [{ type: 'paragraph', value: htmlStr.trim() }];
    };
    ...
    window.editBlog = (id) => {
        ...
        if (blockMatch && blockMatch[1]) {
            try {
                ...
                console.error("Failed to parse blocks, fallback to parsing legacy HTML");
    ...
    /* =========================================
       ROADMAP LOGIC
       ========================================= */
    const ROADMAP_API_BASE = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' 
    ...
    const renderRoadmaps = () => {
        ...
        roadmapTableBody.innerHTML = roadmaps.map(r => `
            <tr>
                <td>
    ...
    document.getElementById("closeRoadmapModalBtn").addEventListener("click", closeRoadmapModal);
    document.getElementById("cancelRoadmapBtn").addEventListener("click", closeRoadmapModal);

    roadmapForm.addEventListener("submit", async (e) => {
        e.preventDefault();
        
        const id = document.getElementById("roadmapId").value;
        const achievementsStr = document.getElementById("roadmapAchievements").value;
        ...
        try {
            ...
            if (res.ok) {
                closeRoadmapModal();
                fetchRoadmaps();
                showToast(id ? "Roadmap updated successfully!" : "Roadmap added successfully!");
            } else {
    ...
    window.editRoadmap = (id) => {
        ...
        document.getElementById("roadmapAchievements").value = r.achievements ? r.achievements.join('\n') : '';
